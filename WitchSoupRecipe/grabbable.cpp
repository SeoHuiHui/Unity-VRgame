...
public class OVRGrabber : MonoBehaviour
{
    ...
    public GameObject pot; // "pot" 모형을 참조할 GameObject 변수
    ...
    protected virtual void GrabBegin()
    {
        ...
        if (closestGrabbable != null)
        {
            ...
            pickingObj = m_grabbedObj.gameObject;
    ...
    {
        // 물건이 특정 위치에 놓여졌는지 확인하는 함수
        float distance = Vector3.Distance(transform.position, pot.transform.position);
        if (distance < 5.0f)
        {
            Debug.Log("good!");
            potSound.Play();
            //audioSource.PlayOneShot(potSound, 1);
            //GameObject successObject = GameObject;
            if (pickingObj) {
                Destroy(pickingObj);
                pickingObj = null;
            }
